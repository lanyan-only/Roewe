$(function() {
	...
	//  var s = "http://flow.xingyuanauto.com/portframe_test/public/index.php/userleave";
      	var s = "https://h5api.xingyuanauto.com/userinfo";
    //  var s = "http://www.h5api.com/userinfo";
	...
	$(".submit").click(function() {
		...
		//项目来源
		var source_end = 1; // 1 pc   0 mobile
		...
		if(!name.match(/^([\u4E00-\u9FA5]{1,4}$)|(^[a-zA-Z]{1,8}$)/)) {
			toMsg("抱歉，姓名需要输入1-4位汉字或八个英文字母");
			return false;
		}

		//性别，必选 1男。2女
		/*var thesex = $("select[name='usersex']").val();
		console.log(thesex)*/
...
//		if($("#agree").is(":checked") == true) {
			...
			$.getJSON(s, {
				...
				if(v.code === 1001) {
					...
					$("select[name='dealerId']").val('');
					$("select[name='cityId']").val('');
					$("select[name='CarId']").val('');
					$("option[value='先生']").attr("selected", "selected");
					//						$("input[value='1']").prop("checked", "checked");
					//						$("input[value='工作日']").prop("checked", "checked");
				} else if(v.code === 1003) {
					toMsg("手机号重复,请重新填写!")
				} else {
					toMsg("注册失败!")
